...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET order_status_id =1,
		...
		//Если был введен новый адрес - добавим его пользователю
		...
		//Если это новый пользователь то адрес уже добавлен
		if(isset($data['new_user'])) $is_find = true;

		//Небыло найдено адреса - добавим
		if(!$is_find){
			$addr = array(
						'customer_id' => (int)$this->customer->getId(),
						'firstname' => $this->db->escape($data['shipping_firstname']),
						...
						'country_id'=> (int)$data['shipping_country_id'],
						'custom_field' => ''
			);
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($this->config->get('config_order_mail')) {
					...
					$mail->setTo('plazamilano@yandex.ru');//$this->config->get('config_email'));
					$mail->setFrom($this->config->get('config_email'));
					$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
					$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
					$mail->setHtml($html);
