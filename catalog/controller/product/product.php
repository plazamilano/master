	...
	public function index() {
		...
		if (isset($this->request->get['path'])) {
			...
			if (isset($this->request->get['product_id'])) {
				...
				$categories = $this->model_catalog_product->getProductCategoryPath($product_id);
...
$data[''] = $this->language->get('');
		...
		$data['options_table'] = array();
		
		foreach($option_sizes as $row){
			$option_header[$row] = $data['option_entry']['entry_'.$row] = $this->language->get('entry_'.$row);
		...
		//==============================================================
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach($option_sizes as $row){
					//$data['options_table'][$option['name']] = $this->language->get('entry_'.$row);
				}
				
				$data['options_table'][$option['name']][$count] = $option_header;
				...
				foreach ($option['product_option_value'] as $option_value) {
					//if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$sizes = $this->model_catalog_product->getOptionValueParams($option_value['product_option_value_id']);
						
						$count++;
						foreach($option_header as $index => $t){
							if($index == 'size'){
								$data['options_table'][$option['name']][$count][$index] = $option_value['name'];
							}else{
								$data['options_table'][$option['name']][$count][$index] = isset($sizes[$index]) ? $sizes[$index] : '';
							}
						}
